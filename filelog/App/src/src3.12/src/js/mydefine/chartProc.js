            ...
            if (data == false) {
                ...
                $http.post('/api/JobTableMactchData', jobDataModel).success(
                  function (data) {
                      ...
                      $scope.datatotal = [
                          { data: $scope.datatotaljob, label: 'A', points: { show: true, radius: 4 }, lines: { show: true, fill: true, fillColor: { colors: [{ opacity: 0.1 }, { opacity: 0.1 }] } } },
                          { data: $scope.datatotaltable, label: 'B', points: { show: true, radius: 2 } }];
